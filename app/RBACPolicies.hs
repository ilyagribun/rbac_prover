...
closurePolicies (RBACPolicies r u pr ap srv inh itr) = RBACPolicies r u pr ap srv (floydWarshallClosure inh) itr
...
checkSSDConstrains policies = all (\(rs, n) -> all (\user -> singleSSD user policies rs n) (users policies)) where
    ...
    ind r = fromJust $ elemIndex r (intToRole policies)
